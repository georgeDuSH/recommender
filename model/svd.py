...
class BiasedFunkSVD(nn.Module):
    """ Matrix Factorization based method, referring to FunkSVD
    """
    def __init__(self, n_user, n_item, k_dim, user_bias, item_bias, global_bias, negative_sampling:bool):
        super(BiasedFunkSVD, self).__init__()
        self.k_dim = k_dim
        self.negative_sampling = negative_sampling # if training with negative sampling
        # bias
        self.bias_user = nn.Parameter(tensor(list(user_bias.values())), requires_grad=False)
        self.bias_item = nn.Parameter(tensor(list(item_bias.values())), requires_grad=False)
        self.bias = nn.Parameter(tensor([global_bias]), requires_grad=False)
        # embeddings of interest
        self.embedding_user = nn.Embedding(n_user, k_dim)
        self.embedding_item = nn.Embedding(n_item, k_dim)
        # init param
